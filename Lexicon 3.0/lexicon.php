...
class plgContentLexicon extends JPlugin
{
	...
	function onPrepareContent(&$row, &$params, $page = 0)
	{
		...
		if (!($type == 'html' && $view == 'article' && !$print)) {
		...
		// Check if we should be active for this category.
		$categories = $this->params->get('categories', null);

		// If one or more categories have been specified, check the category.
		if (!empty($categories)) {
			// If the article category id is not in the array, return true.
			if (!in_array($row->catid, (array)$categories)) {
				return true;
			}
		}

		// Check if the article is uncategorized and should be active.
		if (!$row->catid && !$this->params->get('show_uncat', 0)) {
			return true;
		}

		// Check if we should be active for this user.
		if ($user->get('guest') && !$this->params->get('show_guest', 1)) {
			return true;
		}
		...
		for ($ti = 0, $tc = count($terms); $ti < $tc; $ti++)
		{
			...
			for ($j = 0; $j < $count; $j++)
			{
				...
				if ($wrap === true) {
					...
					$replace	= '<span class="lexicon" title="'.$term.'::'.nl2br($body).'">'.$term.'</span>';
