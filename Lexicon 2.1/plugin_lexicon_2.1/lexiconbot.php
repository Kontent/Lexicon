 ...
 * Definition file
 *
 * Based on Definition Mambot 2 by Martin Brampton modified by RolandD of www.csvimproved.com
 *
 ...
 * @version $Id: definitionbot.php 207 2008-04-20 05:41:27Z Suami $
 */
...
if (!is_callable('getLexiconbotParams')) {
	...
	function makeSearchable (&$param) {
		...
		$database->setQuery('SELECT id,tterm,tdefinition FROM #__lexicon WHERE published=1 ORDER BY length(tterm) DESC');
	...
	function buildContent (&$definitiony, $content, $times) {
		...
		foreach ($definitiony as $gkey=>$entry) {  // run through all lexicon entries
	...
	function linkParams (&$param) {
		...
		if ($param->get('show_image', 1)) $parm['image'] = '<img src="' . JURI::base( true ) . '/plugins/content/lexiconbot/'.$param->get('icon').'" border="0" align="top">';
		else $parm['image'] = '';
	...
	$run = popupCheck($page->text, '{lexicon=enable}', 1, $run);
	$run = popupCheck($page->text, '{mosinfopop=disable}', 0, $run);
	$run = popupCheck($page->text, '{lexicon=disable}', 0, $run);
