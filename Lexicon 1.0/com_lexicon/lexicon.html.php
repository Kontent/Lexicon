...
class HTML_lexicon {
	...
	function deleteHTML ($letter, $catid) {
		...
		if ($lexicon->isEditor()){
			if ($submit) {
				...
				if ($row->tmail) echo "<a href='mailto:$row->tmail'><img src='components/com_lexicon/images/email.png' alt='$row->tmail' hspace='3' border='0'></a>";
				if ($row->tpage) echo "<a href='$row->tpage' target='_blank'><img src='components/com_lexicon/images/homepage.png' alt='$row->tpage' hspace='3' border='0'></a>";
				echo "$row->tdate</td>";
		...
		if ($lexicon->isEditor()){
			...
			if (JRequest::getVar('opt','' )=='del'){
				...
				if ($tcomment) {
					...
					echo "<td width='70%' valign='top'><span class='small'>"._LEXICON_SIGNEDON." $row->tdate<hr></span>$row->tdefinition</td></tr>";
	...
	function submitHTML ($letter, $catid) {
		...
		if (!$lexicon->gl_anonentry AND !$lexicon->isUser()) echo _LEXICON_ONLYREGISTERED;
			...
			<script type="text/javascript">
			function validate(){
				if ((document.lexiconForm.tname.value=='') || (document.lexiconForm.tterm.value=='') || (document.lexiconForm.tdefinition.value=='') || (document.lexiconForm.catid.value=='0')){
