...
$mainframe->registerEvent( 'onPrepareContent', 'plgLexicon' );

function plgLexicon (&$row, &$params, $page=0) {
	$plugin =& JPluginHelper::getPlugin('content', 'lexicon');
	$botParams = new JParameter( $plugin->params );
	$published = JPluginHelper::isEnabled('content','lexicon');
...
class bot_lexiconbot {
	...
	function bot_lexiconbot ($params=null) {
		...
		$switcherJS = <<<SCRIPT
		<script type="text/javascript" src="$this->live_site/plugins/content/lexicon.js"></script>
SCRIPT;
...
// php5
	...
	function loadDefinitions ($find_exact) {
		...
		$this->db->setQuery("SELECT id, tterm, tdefinition FROM #__lexicon WHERE id IN ($deflist)");
	...
	function linkParams () {
		...
		if ($this->botparams->get('show_image', 1)) $this->parm['image'] = '<img src="' . $this->live_site . '/plugins/content/lexicon/info.png" border="0" align="top" alt="lexicon term" />';
		else $this->parm['image'] = '';
		if ($this->botparams->get('show_image', 1)) $bgimage = <<<BG_IMAGE
 background: url("$this->live_site/plugins/content/lexicon/info.png") top left no-repeat;
 padding-left: 16px;
BG_IMAGE;
		else $bgimage = '';
...
LINK_DEFAULT;
	...
	function addSwitcher ($text) {
		...
		$defaults = array('plugins/content/lexicon/icon_off.png','plugins/content/lexicon/icon_on.png','Turn off inline lexicon','Turn on inline lexicon','');
