...
function showOneTerm ($id, $linecolour, &$row1) {
	...
	if ($row1) {	
		...
		echo textwrap($row1->tlexicon,80);
		if ($row1->tcomment) {
...
function lexiconUserSide () {
    ...
    switch ($func) {
		case 'popup':
			...
			$database->setQuery( "SELECT tterm, tlexicon FROM #__lexicon WHERE id='$term' AND published=1");
		...
		case 'entry':
			...
			$tlexicon = $database->getEscaped(JRequest::getVar('tlexicon', ''));
			if ($tname == '' OR $tterm == '' OR $tlexicon == '' OR $catid ==0){
				LexiconHeader($letter, $catid);
				echo '<p><strong>'._LEXICON_VALIDATE.'</strong></p>';
				break;
			}
			if (($lexicon->isEditor()) AND ($id)) {
				$query1 = "UPDATE #__lexicon SET catid='$catid', tname='$tname', tmail='$tmail', tloca='$tloca', tpage='$tpage', tterm='$tterm', tletter=UPPER(SUBSTRING('$tterm',1,1)), tlexicon='$tlexicon' WHERE id=$id";
				$database->setQuery( $query1 );
				$database->query();
			}
			else {
				...
				$query2 = "INSERT INTO #__lexicon SET catid='$catid',tname='$tname',tdate='$tdate',tmail='$tmail', tloca='$tloca', tpage='$tpage', tterm='$tterm', tletter=UPPER(SUBSTRING('$tterm',1,1)), tlexicon='$tlexicon'";
